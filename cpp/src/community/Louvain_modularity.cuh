  ...
  while (tid < nnz) {
    ...
    if (startCluster == endCluster) {
      ...
      for (IdxT i = start; i < end; i++) {
        ...
        if (neighborCluster == startCluster && neighborId == startVertex){
          startEdges += 2 * edge_weights[i];
          endEdges += 2 * edge_weights[i];
        }
      }
...
vertex_t renumber_clusters2(vertex_t num_verts,
                            vertex_t* cluster,
                            cudaStream_t stream) {
  rmm::device_vector<vertex_t> cluster_inv(num_verts);
  rmm::device_vector<vertex_t> temp_cluster(num_verts);
  vertex_t* d_cluster_inv = cluster_inv.data().get();
  vertex_t* d_temp_cluster = temp_cluster.data().get();

//  std::cout << "d_cluster_inv: " << d_cluster_inv << "\n";
//  std::cout << "d_temp_cluster: " << d_temp_cluster << "\n";
//  std::cout << "cluster: " << cluster << "\n";
  ...
  thrust::sort(rmm::exec_policy(stream)->on(stream),
               temp_cluster.begin(),
               temp_cluster.end());
  auto tmp_end = thrust::unique(rmm::exec_policy(stream)->on(stream),
                                temp_cluster.begin(),
                                temp_cluster.end());
  vertex_t new_num_clusters = thrust::distance(temp_cluster.begin(), tmp_end);


  thrust::for_each(rmm::exec_policy(stream)->on(stream),
                   thrust::make_counting_iterator<vertex_t>(0),
                   thrust::make_counting_iterator<vertex_t>(new_num_clusters),
                   [d_temp_cluster, d_cluster_inv] __device__ (vertex_t i) {
                      d_cluster_inv[d_temp_cluster[i]] = i;});

  thrust::for_each(rmm::exec_policy(stream)->on(stream),
                   thrust::make_counting_iterator<vertex_t>(0),
                   thrust::make_counting_iterator<vertex_t>(num_verts),
                   [cluster, d_cluster_inv] __device__ (vertex_t i) {
                      cluster[i] = d_cluster_inv[cluster[i]];});

  return new_num_clusters;
}

template <typename vertex_t, typename edge_t, typename weight_t>
void compute_delta_modularity2(cugraph::experimental::GraphCSRView<vertex_t, edge_t, weight_t> const& graph,
                              vertex_t* d_cluster,
                              weight_t const* d_vertex_weights,
                              weight_t* d_delta_Q,
                              weight_t m2,
                              cudaStream_t stream) {
  vertex_t num_verts = graph.number_of_vertices;
  vertex_t nnz = graph.number_of_edges;

  rmm::device_vector<weight_t> e_c(num_verts, 0.0);
  rmm::device_vector<weight_t> k_c(num_verts, 0.0);
  rmm::device_vector<weight_t> m_c(num_verts, 0.0);

  vertex_t num_clusters = renumber_clusters2(num_verts, d_cluster, stream);

  compute_modularity(num_verts,
                     nnz,
                     num_clusters,
                     graph.offsets,
                     graph.indices,
